package cn.thactclub.fxtz.utils;
...
import cn.thactclub.fxtz.base.Constant;
...
//        boolean isRecordCarshLog = ((Boolean) SpUserUtils.getInstance().get(Constants.Keys.KEY_RECORD_CRASHLOG, true));
//        if (!isRecordCarshLog) {
//            return;
//        }
        ...
        try {
            if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
                // 文件路径
                String path = Constant.Files.CRASH_LOG_PATH;
                File dir = new File(Environment.getExternalStorageDirectory(), path);
                if (!dir.exists()) {
                    dir.mkdirs();
                }
                File file = new File(Constant.Files.CRASH_LOG_FILE_PATH);
                if (!file.exists()) {
                    file.getParentFile().mkdirs();
                    file.createNewFile();
                }
                FileWriter fileWritter = new FileWriter(Constant.Files.CRASH_LOG_FILE_PATH, true);
